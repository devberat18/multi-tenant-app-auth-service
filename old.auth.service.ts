...
type RefreshTokenResult = { token: string; sessionId: number };
type PasswordResetTokenResult = { token: string };
@Injectable()
export class AuthService {
  ...
  ): Promise<RefreshTokenResult | PasswordResetTokenResult> {
  ...
  async changePassword(params: {
    ...
    try {
      const user = await this.authDbService.findById({ userId: params.userId });
      ...
      await this.authDbService.updatePassword({
        userId: user.id,
        password: hashedPassword,
      });

      await this.logoutAllSession(user.id);
